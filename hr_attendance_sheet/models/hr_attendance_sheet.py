    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            for day in all_dates:
                ...
                # جلب الشيفتات من shift_change مرتبة حسب start_date
                shifts = self.env['shift.change'].search([
                    ('employee_id', '=', emp.id),
                    ('state', '=', 'validated'),
                ], order='start_date asc')
                employee_shifts = shifts.filtered(lambda s: s.employee_id == emp)

                if employee_shifts:
                    # اختيار آخر شيفت يبدأ قبل أو في اليوم الحالي
                    active_shift = max(employee_shifts, key=lambda s: s.start_date if s.start_date <= day else fields.Date.from_string('1900-01-01'))
                    calendar_id = active_shift.working_schedule_id
                else:
                    # إذا لم يكن هناك شيفتات، استخدام جدول العمل من العقد
                    if not att_sheet.contract_id or not att_sheet.contract_id.resource_calendar_id:
                        raise ValidationError(_(
                            'Please add working hours to the %s `s contract or a valid shift in Shift Change' % emp.name))
                    calendar_id = att_sheet.contract_id.resource_calendar_id
